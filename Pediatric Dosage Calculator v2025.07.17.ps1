...
function GetWeightInput {
    ...
    do {
        ...
        if ($number -gt 99) {
            $script:statusMessage = "Caution: Weight is higher than expected range"
            $script:statusColor = "DarkRed"
            Write-Host $script:statusMessage -ForegroundColor $script:statusColor
        }
#        $script:statusMessage = ""
...
# 7. New Weight in Kilograms
        7 { 
			...
			if ($script:weightKg -gt 99) {
				$script:statusMessage = "Caution: Updated weight is higher than expected range"
				$script:statusColor = "DarkRed"
				Write-Host $script:statusMessage -ForegroundColor $script:statusColor
			}			
			else {		
...
# 8. New Weight in Pounds
        8 { 
			...
			if ($script:weightKg -gt 99) {
				$script:statusMessage = "Caution: Updated weight: $($script:weightKg) kg (from $lbs lbs) is higher than expected range"
				$script:statusColor = "DarkRed"
				Write-Host $script:statusMessage -ForegroundColor $script:statusColor
			}			
			else {	
