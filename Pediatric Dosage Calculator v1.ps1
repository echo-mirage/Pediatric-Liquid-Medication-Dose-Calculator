...
function IsDuplicateEntry {
    ...
    foreach ($existing in $history) {
        if (
            ...
            $existing.Weight -eq $entry.Weight -and
            $existing.TotalDoseMg -eq $entry.TotalDoseMg
        ) {
            return $true
        }
    }
...
$script:weightKg = GetNumericInput "Enter weight in kilograms"
$script:doseHistory = @()
$script:statusMessage = ""
...
while ($true) {
    ...
    for ($i = 0; $i -lt $medications.Count; $i++) {
        Write-Host " $($i+1). $($medications[$i].Name)"
    }
    Write-Host " 4. Calculate All Preset Medications 1-3"
    Write-Host " 5. Other Medication (Manual Entry)"
    Write-Host " 6. Generate Dosing Chart by Weight"
    Write-Host " 7. New Weight in Kilograms"
    Write-Host " 8. New Weight in Pounds"
    Write-Host " 9. Clear Dose History"
    Write-Host "10. Export Calculations"
    Write-Host "11. Exit"

    $choice = Read-Host "`nEnter choice"
    if ($choice -notmatch '^(1[0-1]|[1-9])$') {
...
# 6. Generate Dosing Chart by Weight
        6 { 
            $name = Read-Host "Enter medication name"
            $mg = GetNumericInput "Enter strength of drug (mg)"
            $ml = GetNumericInput "Enter liquid volume (mL)"
            ...
            $script:statusMessage = "Generated dosing chart for $name"
            $script:statusColor = "Green"
        }
# 7. New Weight in Kilograms
        7 { 
            $script:weightKg = GetNumericInput "Enter new weight in kilograms"
            $script:statusMessage = "Updated weight: $($script:weightKg) kg"
            $script:statusColor = "Green"
        }
# 8. New Weight in Pounds
        8 { 
            $lbs = GetNumericInput "Enter new weight in pounds (auto-converts to kg)"
            $script:weightKg = [math]::Round($lbs / 2.2, 2)
            $script:statusMessage = "Updated weight: $($script:weightKg) kg (from $lbs lbs)"
            $script:statusColor = "Green"
        }
# 9. Clear Dose History
        9 { 
            $script:doseHistory = @()
            $script:statusMessage = "Dose history cleared"
            $script:statusColor = "DarkGreen"
        }
# 10. Export Calculations
        10 { 
